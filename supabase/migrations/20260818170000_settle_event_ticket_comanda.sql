...
BEGIN
  ...
  IF v_rem > 0.001 THEN
    ...
    IF v_has_charge AND v_ticket_left > 0.001 THEN
      SELECT coalesce(sum(amount), 0) INTO v_paid_sum
      FROM public.member_charge_payments
      WHERE charge_id = v_charge.id;
      UPDATE public.member_charges
        SET amount = v_paid_sum,
            status = CASE WHEN v_paid_sum > 0 THEN 'pago' ELSE status END,
            paid_at = CASE WHEN v_paid_sum > 0 THEN v_paid_at ELSE paid_at END,
            cash_entry_id = CASE WHEN v_paid_sum > 0 THEN v_ticket_cash_id ELSE cash_entry_id END
        WHERE id = v_charge.id;
    END IF;
    ...
    UPDATE public.tickets
      SET seller_charge_id = v_new_charge_id
