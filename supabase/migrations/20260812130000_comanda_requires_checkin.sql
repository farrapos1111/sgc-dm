-- Comanda virtual só após check-in do ingresso.

CREATE OR REPLACE FUNCTION public.require_ticket_checkin(_ticket_id uuid)
RETURNS void
LANGUAGE plpgsql
STABLE
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.checkins WHERE ticket_id = _ticket_id
  ) THEN
    RAISE EXCEPTION 'Comanda disponível somente após o check-in no evento';
  END IF;
END;
$$;
