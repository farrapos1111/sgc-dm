-- Excluir evento/ingresso e ações destrutivas: só MC ou presidente da Com. Eventos.
-- (admin_total permanece para suporte do sistema.)
...
REVOKE ALL ON FUNCTION public.can_manage_event_destructive(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.can_manage_event_destructive(uuid) TO authenticated, service_role;

DROP POLICY IF EXISTS events_delete ON public.events;
CREATE POLICY events_delete ON public.events
  FOR DELETE TO authenticated
  USING (public.can_manage_event_destructive(chapter_id));
