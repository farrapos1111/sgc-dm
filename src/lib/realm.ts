...
export const REALMS = ["odm", "fdj", "loja"] as const;
...
export const REALM_CARD_BLURB: Record<Realm, string> = {
  ...
  loja: "Gestão completa para Lojas Maçônicas.",
};
...
const SUBDOMAIN_TO_REALM: Record<string, Realm> = {
  ...
  fdj: "fdj",
  loja: "loja",
  /** Alias legado — redirecionar DNS/hosts para loja.* */
  lodge: "loja",
};
...
/**
 * Apex e www = hub (null). Prefixo odm/fdj/loja = realm.
...
function apexFromHostname(hostname: string): string {
  ...
  return h.replace(/^(www|odm|fdj|loja|lodge)\./, "");
}
...
/**
 * URL para entrar no app de um realm (`/inicio` no host da esfera).
 * Em localhost o Vite não atende `odm.localhost` — fica no mesmo origin.
 * Se já estamos no host desse realm, não recarrega a origem (evita loop no `/`).
 */
export function realmEntryUrl(realm: Realm, fromHostname?: string): string {
  if (typeof window !== "undefined") {
    const hostname = fromHostname ?? window.location.hostname;
    if (isLocalApexHostname(hostname)) return "/inicio";
    const current = resolveRealmFromHost(hostname, getDevRealmOverride());
    if (current === realm) return "/inicio";
  }
  const origin = realmAbsoluteUrl(realm, fromHostname).replace(/\/$/, "");
  return `${origin}/inicio`;
}
